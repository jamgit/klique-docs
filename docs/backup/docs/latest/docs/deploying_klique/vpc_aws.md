...
> Installing the Klique Server on AWS within a VPC, covering security groups, EC2 instance setup, load balancer configuration, and backups.

This guide provides step-by-step instructions for installing the Klique Server on AWS using a Virtual Private Cloud (VPC). 

It covers the following:
* Set up security groups and IAM role
* Create EC2 instance with required disks
* Install dependencies and mount disks
* Deploy Klique version using `docker-compose`
