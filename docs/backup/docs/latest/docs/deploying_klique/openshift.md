...
:::note
Set `klique.cookieDomain` to the common parent domain of the three Klique hostnames (e.g.
...
The Klique server images (`apiserver`, `fileserver`, `webserver`, `kliqueApplications`, `usageAggregator`) run
...
kliqueApplications:
  ...
  containerCustomBashScript: |
    ...
    {extra_bash_init_cmd}
    [ ! -z $KLIQUE_AGENT_NO_UPDATE ] || $LOCAL_PYTHON -m pip install clearml-agent{agent_install_args}
    {extra_docker_bash_script}
    $LOCAL_PYTHON -m clearml_agent execute {default_execution_agent_args} --id {task_id}
  extraEnvs:
    - name: KLIQUE_K8S_GLUE_START_AGENT_SCRIPT_PATH
...
`kliqueApplications.*UrlReferenceOverride` values: they must use `https://` and the externally resolvable
hostnames once TLS termination is in place. Also confirm `klique.cookieDomain` matches the parent domain of the
