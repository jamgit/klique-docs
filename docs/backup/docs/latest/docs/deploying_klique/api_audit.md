# Setting up API Auditing

> How to enable and configure Klique API audit logging via stdout, Fluentd, or Elasticsearch adapters, including per-endpoint and field-level controls.

To facilitate API audit (for security reviews, operational troubleshooting, or usage analysis), the Klique server can provide detailed request/response logs.
...
The Klique server API logs are made available through dedicated adapters:
...
To enable API logging to standard output, set the following environment variables on the Klique API Server container:

* Enable API call logging: `KLIQUE__APISERVER__log_calls=true`   
* Use the logging adapter for stdout JSON output: `KLIQUE__APISERVER__apilog__adapter=raw_log`

:::note[Helm Setup Alternative]
If deploying the server via the `control-plane` Helm chart, logging can be enabled using a single override:
...
Klique also includes a logging adapter capable of sending the API logs to Fluentd, which can then route them to Elastic 
or other backends. This setup is not encouraged, as the default `raw_log` adapter is adequate for most deployments.

### Configuration

* Enable API call logging: `KLIQUE__apiserver__log_calls=true`  
* Enable Fluentd Adapter: `KLIQUE__apiserver__apilog__adapter=logging`  
* Set Fluentd connection settings:

  ```
  KLIQUE__apiserver__apilog__adapters__logging__handler__kwargs__host=<fluentd_host>
  KLIQUE__apiserver__apilog__adapters__logging__handler__kwargs__port=24224
  ```

## Elastic (Alternative)

Klique also includes a logging adapter capable of sending the API logs directly to elastic. This setup is not encouraged,
as the default `raw_log` adapter is adequate for most deployments.

### Configuration

* Enable API call logging: `KLIQUE__apiserver__log_calls=true`  
* Enable elasticsearch Adapter: `KLIQUE__apiserver__apilog__adapter=elastic`  
* Configure Elasticsearch connection and batch settings (optional: You only need to define these if you want to override the defaults):
   ...
   KLIQUE__apiserver__apilog__adapters__elastic__init_timout_sec: 60
  
   # Enable bulk writes
   KLIQUE__apiserver__apilog__adapters__elastic__bulk_enabled: true
  
   # Delay between bulk write flushes (seconds) 
   KLIQUE__apiserver__apilog__adapters__elastic__bulk_write_delay_sec: 5
  
   # Timeout for bulk write operations (seconds)
   KLIQUE__apiserver__apilog__adapters__elastic__bulk_write_timeout_sec: 15  
  
   # Minimum number of log entries required before triggering a bulk write  
   KLIQUE__apiserver__apilog__adapters__elastic__bulk_min_items: 100
   ```
...
If both Klique API and Web server components are running, simply navigating through the UI will generate API calls.
...
```
KLIQUE__apiserver__endpoints___default__log_call=true
KLIQUE__apiserver__endpoints___default__log_data=true
KLIQUE__apiserver__endpoints___default__log_result_data=true
```
...
```
KLIQUE__APISERVER__ENDPOINTS__<endpoint_path>__<log_option>
```
...
```
KLIQUE__apiserver__endpoints__workers__status_report__log_call=true
KLIQUE__apiserver__endpoints__workers__status_report__log_result_data=true
```
...
```
KLIQUE__apiserver__endpoints__workers__status_report__log_data=["queue", "tags"]
```
...
```
KLIQUE__apiserver__apilog__log_fields__auth__enabled=true

KLIQUE__apiserver__apilog__log_fields__headers__enabled=true

KLIQUE__apiserver__apilog__log_fields__version__enabled=true

KLIQUE__apiserver__apilog__log_fields__stats__enabled=true

KLIQUE__apiserver__apilog__log_fields__result__enabled=true

KLIQUE__apiserver__apilog__log_fields__data__enabled=true
```
...
```
KLIQUE__apiserver__apilog__redact_identity=true
```
...
```
KLIQUE__apiserver__apilog__rotation__time=<period>
```
