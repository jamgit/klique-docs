# Application Launch Form Customization

> Overriding Klique application launch form properties (dropdown options, defaults, placeholders) via the app_wizard_overrides server configuration.

An application instance’s launch form is defined in its [configuration file](app_custom.md).
Klique supports modifying some of the form's properties without the need to re-package and deploy the application 
...
In Kubernetes deployments, the `applications.conf` file needs to be mounted into the Klique server pod so that the 
