...
> Configuring multi-tenant SSO login, mapping an external identity provider's tenants to Klique tenants and their sign-in routing.
...
(Keycloak). Each Klique tenant can be associated with a particular external tenant. Currently, only one 
Klique tenant can be associated with a particular external tenant

## Setup IdP/SSO Client in Identity Provider

1. Add the following URL to "Valid redirect URIs": `<klique_webapp_address>/callback_<client_id>`  
2. Add the following URLs to "Valid post logout redirect URIs": 

   ```
   <klique_webapp_address>/login
   <klique_webapp_address>/login/<external tenant ID>
   ```
3. Make sure the external tenant ID and groups are returned as claims for each user

## Configure Klique to use Multi-Tenant Mode

Set the following environment variables in the `control-plane` helm chart under the `apiserver` section:  
* To turn on the multi-tenant login mode:

  ```
  - name: KLIQUE__services__login__sso__tenant_login
    value: "true"
  ```
* To hide any global IdP/SSO configuration that's not associated with a specific Klique tenant:  

  ```
  - name: KLIQUE__services__login__sso__allow_settings_providers
    value: "false"
  ```
...
## Setup IdP for a Klique Tenant

To set an IdP client for a Klique tenant, you’ll need to set the Klique tenant settings and define an identity provider:

1. Call the following API to set the Klique tenant settings:
   ...
   curl $APISERVER_URL/system.update_company_sso_config -H "Content-Type: application/json" -u $APISERVER_KEY:$APISERVER_SECRET -d'{  
     ...
     "sso": {  
       ...
       "group_mapping": {  
         "IDP group name1": "Klique group name1",  
         "IDP group name2": "Klique group name2"  
       },  
       "admin_groups": ["IDP admin group name1", "IDP admin group name2"]  
     }}'  
   ```
2. Call the following API to define the Klique tenant identity provider:
   ...
   * On logout from Klique, the user is also logged out from the Identity Provider  
   ...
   * Group integration is turned on and a user will be allowed to log in if any of the groups s/he belongs to in the 
     IdP exists under the corresponding Klique tenant (this is after group name translation is done according to the Klique tenant settings)
