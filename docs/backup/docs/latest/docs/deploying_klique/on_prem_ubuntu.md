...
> Installing the Klique Server on a single Ubuntu server with Docker Compose, including prerequisites, security, backups, and troubleshooting.

This guide provides step-by-step instruction for installing the Klique Server on a single Linux Ubuntu server.

## Prerequisites
The following are required for the Klique on-premises server:
...
- User for running Klique services with administrator privileges  
- Ports 8080, 8081, and 8008 available for the Klique Server services

In addition, make sure you have the following (provided by Klique):

- Docker hub credentials to access the Klique images  
...
for all Klique data and configuration files, is defined as expected here (e.g. `grep CLEARML_ROOT constants.env`)

## Installing Klique Server 
### Preliminary Steps 
...
1. Remove any previous installation of Klique Server:
...
1. Copy the following Klique configuration files to `${CLEARML_ROOT}`:
   * `constants.env`
   * `docker-compose.override.yml`
   * `docker-compose.yml`

1. Create an initial Klique configuration file `${CLEARML_ROOT}/config/onprem_poc/apiserver.conf` with a fixed user:
...
1. Log into the Docker Hub repository using the username and password provided by Klique:
...
services:
  apiserver:
    environment:
      - KLIQUE__apiserver__auth__cookies__secure=false
```

Only the following ports should be open to any location where a Klique client (`clearml-agent`, SDK, or web browser) may operate:
...
Applications are plugins that extend the functionality of the Klique Server, letting users manage ML workloads and
automate recurring workflows without code. Install them on top of the Klique Server:

- [Applications (On-Prem / VPC)](app_install_ubuntu_on_prem.md): install Applications on the same server or VPC as the
  Klique Server.
- [Applications (External Server)](app_install_ex_server.md): install Applications on a separate server from the
  Klique Server.

## Backups

To ensure data consistency and prevent corruption during a restore, it is recommended to regularly back up the storage 
components where Klique saves its data. See [Backups](extra_configs/backups.md) for more information.
