# Changing Klique Artifacts Links

> Using the fix_fileserver_urls.py script to update artifact storage references in Klique's MongoDB and ElasticSearch after migrating external storage.

This guide describes how to update artifact references in the Klique server.

By default, artifacts are stored on the file server; however, an external storage such as AWS S3, Minio, Google Cloud 
Storage, etc. may be used to store artifacts. References to these artifacts may exist in Klique databases: MongoDB and ElasticSearch.  
This procedure should be used if external storage is being migrated to a different location or URL.

:::important
This procedure does not deal with the actual migration of the data--only with changing the references in Klique that 
point to the data.
:::
...
It is highly recommended to back up the Klique MongoDB and ElasticSearch databases before running the script, as the 
...
1. Access the `apiserver` Docker container:  
   ...
   * In Kubernetes:
      ...
      ```commandline
      kubectl exec -it -n klique <klique-apiserver-pod-name> -- bash
      ```

1. Navigate to the script location in the jobs management folder:

   ```commandline
   cd /opt/clearml/apiserver/server/jobs/management
   ```
...
1. Run the following command:
    ...
    or that the script provided by Klique was copied into the container).
    :::
