# Application Installation on On-Prem and VPC Servers

> Installing Klique applications on on-premises and VPC docker-compose servers using upload_apps.py, including air-gapped image registry conversion.

Klique Applications are like plugins that allow you to manage ML workloads and automatically run recurring workflows 
without any coding. Applications are installed on top of the Klique Server.
...
* Applications Service: Make sure that the applications agent service is up and running on your server:
  * If you are using a `docker-compose` solution, make sure that the `apps-agent` service is running.
  * If you are using a Kubernetes cluster, check for the `control-plane-apps` component.
...
## Installing on Klique Server
The `upload_apps.py` script handles uploading the app packages to the Klique Server. It requires Python3.
...
the credentials of an admin user, which can be generated in the Klique web application.
