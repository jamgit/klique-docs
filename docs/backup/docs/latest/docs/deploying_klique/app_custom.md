...
> Building and installing custom Klique applications: config file format, launch form wizard fields, dashboard layout, and the custom apps agent.

The following is a guide for creating and installing custom applications on on-premises Klique servers.
Klique applications are Python programs that are run as tasks whose UI--input form and output dashboard--is 
...
* `description` - Short description of the application to be displayed in the Web UI
* `icon` (*Optional*) - Small image to display in the Web UI as an icon for the application. Can be a public web url or an image in the application’s assets directory (described below)
...
* `category` (*Optional*) - Way to separate apps into different tabs in the Web UI
...
* `choices` - for dropdown - Can be either an array of hard-coded options, for example: `["Option 1","Option 2"]`, or a Klique object, such as task, project, queue to choose from. The following should be specified:
...
the Web UI. They typically contain icons, empty-state HTML, and any other object required. Assets are stored in 
...
### Custom Apps Agent
Custom applications require a separate agent then the Klique built-in applications since their code is downloaded from 
...
* In the `apiserver` service section, add the following lines in the environment to create a user for handling the custom-apps:
  ```
  - KLIQUE__secure__credentials__custom_apps_agent__user_key="${CUSTOM_APPS_AGENT_USER_KEY}"
  - KLIQUE__secure__credentials__custom_apps_agent__user_secret="${CUSTOM_APPS_AGENT_USER_SECRET}"
  - KLIQUE__secure__credentials__custom_apps_agent__role="admin"
  ```
...
* `<apiserver url>` can be something like `https://api.my-server.example.com` or `http://localhost:8008` if running on the server.
* `--key` and `--secret` are key/secret credentials of any Klique admin user. These can be generated in the Web UI.
