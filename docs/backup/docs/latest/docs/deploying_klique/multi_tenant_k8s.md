...
> Installing a Klique multi-tenant service on Kubernetes, including tenant creation, SSO sign-in rules, and feature and group configuration.

This guide provides step-by-step instructions for installing a Klique multi-tenant service on a Kubernetes cluster.

## Klique Server

To install the Klique Server, follow the [Klique Kubernetes Installation guide](k8s.md).

Update the Server's `control-plane-values.override.yaml` with the following values:

```yaml
apiserver:
  extraEnvs:
    - name: KLIQUE__services__organization__features__user_management_advanced
      value: "true"
    - name: KLIQUE__apiserver__company__unique_names
      value: "true"
```
...
This section explains how to create a new tenant (company) in the Klique Server using the Klique API.
...
* **Define variables to use in the next steps:**
   ...
   :::note
   The apiserver key and secret should be the same as those used for installing the Klique Server Chart.
   :::
...
* **Create Credentials for the new Admin User:**
   ...
   :::note 
   You can use this set of credentials to set up a [Klique Orchestrator](../orchestrator/deployment_k8s.md#installing-the-orchestrator) 
   or [App Gateway](../whats_klique/appgw.md) for the new Tenant.
   :::
...
* **Route an email to a tenant based on the email's domain:**
   ...
   * `<USERS_EMAIL_DOMAIN>` is the email domain set up for users to access through SSO (e.g. `"acme.io"`, `"acme-labs.com"`).
...
The server's `control-plane-values.override.yaml` can control the features available to 
...
apiserver:
  extraEnvs:
    - name: KLIQUE__services__auth__default_groups__users__features
      value: "[\"applications\"]"
```
...
To install and configure the Klique components that run user workloads, refer to:
* [Klique Orchestrator](../orchestrator/deployment_k8s.md#installing-the-orchestrator)
* [App Gateway](../whats_klique/appgw.md).

:::note
Make sure to set up the Orchestrator and App Gateway using a Tenant's admin user credentials created with the Tenant creation APIs 
...
* Deploy a dedicated Klique Orchestrator and AI Application Gateway in each Namespace.
* Configure the tenant's orchestrator and gateway with credentials created on the Klique Server by a user of the same tenant.
...
The following setting in `control-plane-values.override.yaml` defines a global feature whitelist. It overrides all tenant-specific 
...
Klique groups are used to control user permissions and access to the platform. 
This section describes the types of groups available and how to configure them--especially cross-tenant groups.

#### Group Types

Klique utilizes several types of groups:
* **Built-in Groups** (default in every Klique installation):
...
* **Tenant-Specific Groups (via UI)** - Additional tenant-specific groups can be created
  directly through the Web UI (under **Settings > Users & Groups**). Users can be assigned to these groups via 
  the UI. For more information, see [Users & Groups](../webapp/settings/webapp_settings_users.md).
* **Cross-Tenant Groups (Configuration)** - These groups are defined centrally in the Klique configuration files 
...
```
KLIQUE__services__auth__default_groups__<GroupName>__<Property>
```
...
| ------------ | -------------------------------------------------------------------------- | ------------------------------------ | ------------------------------- |
| `id`         | A unique identifier for the group. This **must** be a standard UUID (Universally Unique Identifier). You can generate one using various online tools or libraries. | `KLIQUE__services__auth__default_groups__<GroupName>__id` | `"abcd-1234-abcd-1234"` |
| `name`       | Display name for the group (should match `<GroupName>` used in the variable path) | `KLIQUE__services__auth__default_groups__<GroupName>__name` | `"My Group Name"`, `"MLOps Team"`  |
| `features`   | JSON list of features to enable for this group . For the complete list of available features, see [Available Features](#available-features). Note that the features must be defined for the tenant or for the entire server in order to affect the group. By default, all the features of the tenant are available to all users. | `KLIQUE__services__auth__default_groups__<GroupName>__features` | `'["applications", "experiments", "pipelines", "reports", "show_dashboard", "show_projects"]'` (Note the single quotes wrapping the JSON string if setting via YAML/environment variables). |
| `assignable` | Whether admins can assign users to this group from the Klique Web UI (`true`/`false`). If `false`, group membership is managed externally or implicitly. | `KLIQUE__services__auth__default_groups__<GroupName>__assignable` | `"false"`                       |
| `system`     | Always set to `"false"` for custom groups | `KLIQUE__services__auth__default_groups__<GroupName>__system` | `"false"`                       |
...
- name: KLIQUE__services__auth__default_groups__my_group_name__id
  value: "abcd-1234-abcd-1234" # Replace with a newly generated UUID

# Group name for my_group_name
- name: KLIQUE__services__auth__default_groups__my_group_name__name
  value: "My Group Name"

# List of features for my_group_name
- name: KLIQUE__services__auth__default_groups__my_group_name__features
...
- name: KLIQUE__services__auth__default_groups__my_group_name__assignable
  value: "false"

# Always false for custom groups
- name: KLIQUE__services__auth__default_groups__my_group_name__system
  value: "false"
```

### Feature Assignment Strategy

Klique uses a feature-based permission model, where each user’s access is determined by the groups they belong to. 
...
```
KLIQUE__services__organization__features__<FeatureName>
```
...
- name: KLIQUE__services__auth__default_groups__admins__features
...
settings, you can create a flexible and secure permission model tailored to your Klique deployment. 

:::important
Remember to restart the relevant Klique services after applying configuration changes.
:::
...
The Klique Server and Klique Orchestrator support different queue modes:

- `global` (default) - A single Apps Agent on the server. The application's controllers will start on the server.
- `per_tenant` - Multiple Apps Agents, one per tenant (will need `orchestratork8sglue.appsQueue.enabled=true` on Agents). The 
   application's controllers will start on the worker.

Configure the Server's `control-plane-values.override.yaml`:
...
Configure the orchestrator's `orchestrator-values.override.yaml`:
...
:::note
This feature requires the orchestrator to be configured using internal admin credentials as previously mentioned in the 
"Create an Admin User for the new tenant" section, making sure to pass `"internal":"true"` and using the output
credentials for `klique.orchestratork8sglueKey` and `klique.orchestratork8sglueSecret` (or `klique.existingOrchestratork8sglueSecret`).
:::
...
| `permissions` | Enables editing of Role-Based Access Control (RBAC) rules. |  |
| `applications` | Allows users to work with [Klique Applications](../webapp/applications/apps_overview.md) (viewing, running). | Excludes management operations (upload/delete). |
| `app_management` | Allows application management operations: upload, delete, enable, disable. |  |
...
| `queue_management` | Allows create, update, and delete operations on queues. |  |
...
| `config_vault` | Enables the [configuration vaults](../webapp/settings/webapp_settings_profile.md#configuration-vault) feature. |  |
| `pipelines` | Enables access to Pipelines (building and running). |  |
| `reports` | Enables access to [Reports](../webapp/webapp_reports.md). |  |
| `resource_dashboard` | Enables access to the [orchestration dashboard](../webapp/webapp_orchestration_dash.md) feature. |  |
| `sso_management` | Enables the SSO (Single Sign-On) configuration wizard. |  |
| `service_users` | Enables support for creating and managing service accounts (API keys). |  |
...
| `model_serving` | Enables access to the [Model Endpoints](../webapp/webapp_model_endpoints.md) feature. |  |
| `show_dashboard` | Makes the "Dashboard" menu item visible in the UI sidebar. |  |
| `show_model_view` | Makes the "Models" menu item visible in the UI sidebar. |  |
| `show_projects` | Makes the "Projects" menu item visible in the UI sidebar. |  |
