# Application Instance Visibility Policy

> How to configure Klique application instance visibility policies (all, owner, or group) globally, per application, or per tenant.

By default, all users can see all application instances in the Klique UI, but admins can change this policy per 
...
the `services.conf`, or the equivalent environment variable `KLIQUE__services__applications__app_instance_access__allowed`. 
...
For example, to set the default visibility to group on Kubernetes, add the following to control-plane-values.override.yaml: 

```
apiserver:
 extraEnvs:
   - name: KLIQUE__services__applications__app_instance_access__allowed
     value: "group"
```
...
```
KLIQUE__services__applications__app_instance_access__applications__<app_id>__allowed
```
...
apiserver:
  extraEnvs:
    - name: KLIQUE__services__applications__app_instance_access__applications__hpo_app__allowed
      value: "owner"
```
...
Use the [`system.update_company_app_instance_access`](../references/api/system.md#post-systemupdate_company_app_instance_access) 
