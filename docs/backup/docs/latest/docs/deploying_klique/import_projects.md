...
> Exporting projects from a ClearML open source server or a Klique Server and importing them into a Klique Server, with file server artifacts.

When migrating to a Klique Server, whether from a ClearML open source server or from another Klique Server, you may 
need to transfer projects. This is done using the `data_tool.py` script. This utility is available in the `apiserver` 
Docker image, and can be used for exporting and importing project data from either server type.

This guide covers the following:
* Exporting data from a ClearML open source server or a Klique Server  
* Importing data into a Klique Server 
* Handling the artifacts stored on the file server. 

:::note 
Export instructions differ between ClearML open source servers and Klique Servers. Make sure you follow the guidelines 
...
Note that artifacts stored in the ***file server*** should be copied manually if required (see [Handling Artifacts](#handling-artifacts)).

### Exporting Data from a ClearML Open Source Server

#### Preparation

* Make sure the `apiserver` is at least ClearML open source server version 1.12.0.  
...
* In Kubernetes:
  ...
  ```commandline
  kubectl exec -it -n <klique-namespace> <klique-apiserver-pod-name> -- bash 
  ```
...
### Exporting Data from a Klique Server

#### Preparation

* Make sure the `apiserver` is at least Klique Server version 3.18.0.  
* Note that any `pending` or `running` tasks will not be exported. If you wish to export them, make sure to stop/dequeue 
before exporting.
...
This section explains how to import the exported data into a Klique Server.

### Preparation

* It is highly recommended to back up the databases before importing data, as import injects data into the 
...
1. Run the data tool script in *import* mode:
   ...
   * `company_id`- The default company ID used in the target deployment. Inside the `apiserver` container you can 
     usually get it from the environment variable `KLIQUE__APISERVER__DEFAULT_COMPANY`.   
...
***Artifacts*** refers to any content which the Klique Server holds references to. This can include: 
* Dataset or Hyper-Dataset frame URLs
* Artifact URLs 
* Model snapshots
* Debug samples

Artifacts may be stored in any external storage (e.g., AWS S3, minio, Google Cloud Storage) or in the file server.  
* If the artifacts are **not** stored in the file server, they do not need to be moved during the export/import process, 
as the URLs registered in database objects pointing to these artifacts will not change.  
* If the artifacts are stored in the file server, then the file server content must also be moved, and the URLs 
  in the databases must point to the new location. See the file server export instructions for your source server below.

### Exporting File Server Data from a ClearML Open Source Server
...
The entire projects' folders content should be copied to the target server (see [Importing File Server Data](#importing-file-server-data)).

### Exporting File Server Data from a Klique Server
...
[Changing Klique Artifacts Links](change_artifact_links.md) for information on how to fix the URLs.  

## Updating Embedded Links in Reports and Projects Overview

After migrating your data to a new server, you should update embedded URLs in your project [overviews](../webapp/webapp_project_overview.md) 
...
reports that contain links to objects in the old URL, you will need to update the links manually using the Web UI.
:::
...
**Note:** To find the container or pod name, you can use `docker ps --filter "name=mongo"` for Docker or `kubectl get pods -n klique | grep mongo` for Kubernetes.
...
// Example new URL: "https://klique.my-new-domain.com"
