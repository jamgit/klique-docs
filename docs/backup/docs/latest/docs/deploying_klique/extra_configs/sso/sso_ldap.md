...
* `KLIQUE__apiserver__auth__fixed_users__enabled=true`
* `KLIQUE__apiserver__auth__fixed_users__provider=ldap` 
* `KLIQUE__apiserver__auth__fixed_user_providers__ldap__server="<ldap server address. For example: ipa.demo1.freeipa.org>"`
* `KLIQUE__apiserver__auth__fixed_user_providers__ldap__base_dn="<base dn searching users under. For example: dc=demo1,dc=freeipa,dc=org>"`
* `KLIQUE__apiserver__auth__fixed_user_providers__ldap__default_company=<company id>`
* `KLIQUE__secure__fixed_user_providers__ldap__dn="<admin dn>"`
* `KLIQUE__secure__fixed_user_providers__ldap__password="<admin pwd>"`
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__search_filter: "(&(objectclass=user)(sAMAccountName={0}))"
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__use_tls=true
```
...
1. Set the path to the certificate so the apiserver can find it:
   ...
   ``` 
   KLIQUE__apiserver__auth__fixed_user_providers__ldap__tls__ca_certs=/opt/allegro/config/onprem_poc/<certificate_file>
   ```
  
   You can skip certificate validation by setting the following variable:

   ```
   KLIQUE__apiserver__auth__fixed_user_providers__ldap__tls__validate_cert=false 
   ```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__tls__client_cert=/opt/allegro/config/onprem_poc/<client_certificate_file>
KLIQUE__apiserver__auth__fixed_user_providers__ldap__tls__client_key=/opt/allegro/config/onprem_poc/<client_private_key_file>
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__attributes__name=<user name attribute>
KLIQUE__apiserver__auth__fixed_user_providers__ldap__attributes__mail=<user email attribute>
KLIQUE__apiserver__auth__fixed_user_providers__ldap__attributes__<some_custom_attrib>=<custom attribute name in ldap>
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__admin_users=["user1_id","user2_id"]
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__groups__enabled=true
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__groups__prohibit_user_login_if_not_in_group=true
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__groups__admins=["<cn name of group1>","<cn name of group2>"]
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__case_sensitive=false
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__group_name_parsing="none"
```
...
```
KLIQUE__apiserver__auth__fixed_user_providers__ldap__default_company
```
