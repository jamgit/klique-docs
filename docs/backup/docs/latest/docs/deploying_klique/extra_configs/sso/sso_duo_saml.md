...
1. Set the following environment variables:
   ...
   * `KLIQUE__secure__login__sso__saml_client__duo__entity_id=clearml`
   * `KLIQUE__secure__login__sso__saml_client__duo__idp_metadata_file=<path to the metadata file>`
   * `KLIQUE__secure__login__sso__saml_client__duo__default_company=<company_id>`
