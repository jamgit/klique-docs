# Customizing the Klique Web UI

> The environment variable parameters for customizing and white labeling the Klique Web UI, including branding, theming, and login behavior.

Klique enables organizations to offer a custom branded Web UI through a flexible customization mechanism.

These customizations are implemented through container start-time parameters, passed as environment variables when 
launching the Klique UI container. For example:
...
The following parameters can be used to customize the Klique UI:
...
* `displayTips?: boolean;`  
  ...
  * **Possible Values:**  
    * `true`: Display Klique built-in tips and suggestions in the UI  
    * `false`: Hide tips for a cleaner, more white-label experience.

* `blockUserScript?: boolean;`
  * Controls whether user-provided and third-party scripts are allowed to run in the Klique Web UI.
  * **Possible Values:**
...
* `defaultTheme?: 'light' | 'dark';`  
  ...
  * **Possible Values:** `'light'`, `'dark'`  
* `customStyle?: string;`  
  * URL to a custom CSS file that overrides default Klique styles. This allows full control over appearance, including 
    fonts, colors, spacing, and element visibility.  
  * For example: `"<https://yourcdn.com/path/to/your-custom-klique-styles.css>"`.
...
* `externalAddUsers?: boolean;`  
  ...
  * Indicates if user addition is managed externally (e.g., via an API or a separate identity provider) or through the 
  Klique UI.  
  * **Possible Values:**  
    * `true`: User addition is managed externally. The Klique UI options for adding users are hidden, directing users 
    to the appropriate external system.  
    * `false`: User addition is managed through the Klique UI
...
* `branding?: { faviconUrl?: string; logo?: string; logoSmall?: string };`  
  * Replaces Klique's default branding elements with custom ones.  
...
* `docsLink?: string;`  
  ...
  * Replaces the default link to Klique's documentation with a custom URL.  
...
These parameters are passed as environment variables when launching the Klique UI Docker container.
...
```
services:
  clearml-webserver: # Name of your Klique UI service
    image: your-clearml-ui-image-name:latest # Replace with your actual Klique UI image
    ...
    environment:
      ...
      - WEBSERVER__customStyle=https://yourcdn.com/path/to/your-custom-klique-styles.css
...
webserver:
  extraEnvs:
    ...
    - name: WEBSERVER__customStyle
      value: "https://yourcdn.com/path/to/your-custom-klique-styles.css"
