...
> The Orchestration Dashboard, which monitors available and in-use compute resources, resource groups, and usage history.
...
clicking  on a column, and the 
relevant filter appears. To clear all active filters, click .

Hover over a worker and click  
...
Click  to define the threshold values. 
