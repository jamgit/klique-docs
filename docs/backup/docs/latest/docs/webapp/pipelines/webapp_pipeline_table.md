# The Pipeline Run Table

> The pipeline runs table, its columns, filtering and customization options, run creation, and run actions.
...
| **STATUS** | Pipeline run's status. See a list of the [task states and state transitions](https://clear.ml/docs/latest/docs/fundamentals/task#task-states). For Running, Failed, and Aborted runs, you will also see a progress indicator next to the status. See [here](https://clear.ml/docs/latest/docs/pipelines/). |  String |
...
browser bookmark and shared with other Klique users.
...
In the **Scalars** view, click  to access [scalar plot tools](../webapp_exp_track_visual.md#scalar-plot-tools).
