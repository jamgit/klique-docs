...
> Administrator vaults distribute client configuration, UI storage credentials, or SSH server keys to user groups, tracked in an activity log.
...
Client configuration vaults extend and/or override entries in the local [clearml.conf](https://clear.ml/docs/latest/docs/configs/clearml_conf)
where a task is executed. Vault values will be applied to tasks run by members of the designated user groups. 

New entries will extend the configuration in the local [clearml.conf](https://clear.ml/docs/latest/docs/configs/clearml_conf). 
...
Hover over a vault in the table and click 
