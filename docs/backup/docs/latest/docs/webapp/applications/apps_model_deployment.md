...
> LLM deployment as vLLM inference services behind a secure endpoint, covering multi-model hosting, LoRA adapters, and speculative decoding.
...
If the Klique AI Application Gateway is not available, the model endpoint might not be accessible.
...
* **Service Project (Access Control)**: The Klique project where the app instance is created. Access is determined by 
  project-level permissions (i.e. users with read access can use the app).
* **Queue**: The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the vLLM Model Deployment app 
instance task will be enqueued. Make sure an agent is assigned to that queue.

  :::tip[Multi-GPU inference]
  To run multi-GPU inference, ensure the queue's pod specification (from the base template and/or `templateOverrides`) defines multiple GPUs. See [GPU Queues with Shared Memory](../../orchestrator/custom_workload.md#example-gpu-queues-with-shared-memory)
...
* **Model Configuration**: Configure the behavior and performance of the model engine.
  ...
  * Model: A Klique Model ID or a HuggingFace model name (e.g. `openai-community/gpt2`)
  ...
  * Tokenizer: A Klique Model ID or a Hugging Face tokenizer  
