...
> The Embedding Model Deployment application, which serves embedding models over a secure network endpoint with multi-model hosting.
...
* **Service Project** - Klique Project where your Embedding Model Deployment app instance will be stored
* **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the Embedding Model 
...
* **Model Configuration**
  * Model - A Klique Model ID or a Hugging Face model name (e.g. `openai-community/gpt2`)
