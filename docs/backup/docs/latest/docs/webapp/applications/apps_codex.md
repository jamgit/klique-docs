...
> The Codex application, which launches a remote VS Code session pre-configured with OpenAI's Codex CLI.

The Codex app launches a detachable remote VS Code session pre-configured with OpenAI's Codex CLI, ready to use as
soon as the session starts.

The Codex app offers workspace management features, allowing you to store, sync, and restore interactive workspaces
across sessions. This ensures your work, including `~/.codex` session state, is preserved and can be easily accessed
in future sessions.

The Codex session is set up using a [Klique Orchestrator](../../orchestrator.md). When configuring an app instance, select a queue, and the 
...
the session ID and a link to open the VS Code session with Codex ready to use.
...
Once you start a Codex instance, you can view the following information in its dashboard:
...
* Current session ID
* Open Codex - Link to the VS Code session with Codex ready to use
...
already available. Ask Codex to work with your Klique projects, tasks, models, and apps directly, with no manual
...
## Codex Instance Configuration
When configuring a new Codex instance, you can fill in the required parameters or reuse the configuration of 
...
* **Compute Resource (Queue)** – The [Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) serviced by the Klique Orchestrator that will run the Codex session.
* **Codex Session Project** - The project where the app instance is created. Access follows project-level 
  permissions: users with read access to the project can use the app.
* **AI Provider**
  * Set a Custom OpenAI API Key - Select to provide a custom OpenAI API key for the session instead of the default key from
  ...
  * Vendor Endpoint - Route model calls through the [LLM Gateway](../settings/webapp_settings_llm_gateway.md) instead 
    of calling OpenAI's API directly. Leave blank to call the API directly. 
  * Update Codex at Session Start - When enabled, updates the Codex CLI to the latest version when the 
...
* **Advanced Options**
  ...
  * Continue from Previous Session - Select the ID of a previous Codex session to restore
  * Container Image - Base runtime image for the session container. Every option includes both Python and Node.js 
  ...
  * Environment Variables - One `KEY=value` per line. Injected into every shell inside the session via 
    `/etc/profile.d/codex.sh`. Lines starting with `#` are ignored.
  ...
  * Persistent Workspace Path - Directory snapshotted when the session closes and loaded back in when a new 
    ...
    `~/.codex` state, and cloned repos all persist across sessions. Leave blank for an ephemeral session.
