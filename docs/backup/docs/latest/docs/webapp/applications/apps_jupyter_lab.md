...
> A remote Jupyter Lab session launched over SSH, with workspace persistence and configuration options for git, containers, and idle shutdown.
...
The Jupyter Lab session is set up using a [Klique Orchestrator](../../orchestrator.md). When configuring an app instance, 
select a queue, and the Klique Orchestrator servicing that queue will download and launch the IDE on its machine. When the server 
...
* **Queue** - The queue serviced by the Klique Orchestrator that will execute the Jupyter Lab session
