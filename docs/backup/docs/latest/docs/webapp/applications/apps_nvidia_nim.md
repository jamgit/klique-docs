...
> NVIDIA NIM models launched in their vendor containers behind a secure endpoint, requiring an NGC API key.
...
If the AI Application Gateway is not available, the model endpoint might not be accessible.
...
:::tip[Embedding Klique Visualization]
You can embed plots from the app instance dashboard into [Klique Reports](../webapp_reports.md) and other third-party platforms that support embedded content
...
* **Compute Resource (Queue)**: The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the NIM app instance task will be enqueued. 
  ...
  * Runs in [Docker mode](https://clear.ml/docs/latest/docs/clearml_agent/clearml_agent_execution_env/#docker-mode) if it is a bare-metal agent
  ...
  See [NIM Cache](../../orchestrator/nim_caching.md) on how to configure persistent caching for NIM 
