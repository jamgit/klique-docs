...
> Distributed training job orchestration across a master node and worker nodes using frameworks like torchrun or Accelerate.

The Multi-Node Trainer application enables users to run distributed training jobs across multiple nodes. The application 
orchestrates a master node and one or more worker nodes through a Klique queue and automatically configures the 
...
:::note
Administrators can [customize](../../deploying_klique/app_launch_form_custom.md) 
...
* **Compute Resource (Queue)** – The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) serviced by the Klique Orchestrator that will run the training nodes. The queue must support executing multiple tasks simultaneously (e.g. serviced by an autoscaler or multiple agents).  
...
* **Base Task ID** (optional) – An existing Task ID to use as a base configuration. If set, the task is cloned, using its Git and container settings.
* **Git**
