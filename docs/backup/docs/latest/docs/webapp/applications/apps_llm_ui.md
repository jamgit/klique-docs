...
> A chat interface to models served by the vLLM, Llama.cpp, and SGLang deployment apps, including RAG knowledge bases.

Use the Klique LLM UI application to launch a visual chat interface to a deployed model.

The app instance uses endpoints of models deployed through the Klique [vLLM Model Deployment](apps_model_deployment.md), 
...
model. If the Klique AI Application Gateway is not available, the model endpoint might not be accessible. For more 
information, see [AI Application Gateway](../../whats_klique/appgw.md).
:::
...
* Queue: The queue serviced by the Klique Orchestrator which will execute the application instance.
