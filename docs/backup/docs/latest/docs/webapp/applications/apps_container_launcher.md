...
> The Containerized Application Launcher, which remotely runs application containers with network routing and persistent workspaces.
...
If the Klique AI Application Gateway is not available, the application container might not be accessible.
...
* App status indicator  
  *  - Remote containerized app is setting up  
  *  - Remote containerized app is active  
  *  - Remote containerized app is idle  
  *  - Remote containerized app is stopped  
* Idle time  
...
* Endpoint - A direct URL to the running containerized application service, intended for programmatic access. This endpoint 
  does not include browser-based authentication, so you must manually provide an [Application Gateway](../../whats_klique/appgw.md) 
...
* **Application instance project** - Klique Project where your app instance will be stored  
...
* **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the containerized app launcher instance task will be enqueued (make sure an agent is assigned to that queue).  
