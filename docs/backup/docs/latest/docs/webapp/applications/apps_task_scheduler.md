...
> The Task Scheduler application for scheduling one-shot and recurring task execution, its dashboard, and configuration options.

The Task Scheduler Application lets you schedule tasks for one-shot and/or periodic execution at specified times. 
...
Each scheduling job is configured with tasks and a scheduling specification for each task: the time 
...
* **Scheduled Tasks** 
  ...
  * **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which scheduled tasks are enqueued (make sure an agent is assigned to that queue)
