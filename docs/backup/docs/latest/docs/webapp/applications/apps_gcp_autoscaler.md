...
> The GCP Autoscaler application, which spins GCP VM instances up and down based on queue demand and a configured resource budget.
...
Each resource type is associated with a Klique [queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) whose 
...
* **Git Configuration** - Git credentials with which the Klique Executor running on your VM instances will access your 
...
* **Base Docker Image** (optional) - Available when `Use docker mode` is selected. Default Docker image in which the Klique Executor will run. Provide an image stored in a 
...
* **Ignore vault parsing errors** - If not selected, the autoscaler will abort if encountering errors when loading vaults 
  ...
  tasks run by the autoscaler. For more information, see [Configuration Vault note](#configuration-vault).
* **Init Script** (optional) - A bash script to execute after launching the VM instance
* **Additional ClearML Configuration** (optional) - A `clearml` configuration file to use by the Klique Executor when executing your tasks
