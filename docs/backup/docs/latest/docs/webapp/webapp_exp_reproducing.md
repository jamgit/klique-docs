...
> How to reproduce task runs in the WebApp by cloning or resetting tasks and re-enqueuing them for execution.
...
1. In the `Clone Task` modal, set the following:
   ...
   * Name - The name for the new task. 
     ...
     This default name can be customized via UI **Settings** page **>** [**Task clone name template**](settings/webapp_settings_ui_customization.md#task-clone-name-template)
     ::: 
   
   * Set `<cloned_task>` as parent - Select to set this task as the new task's parent
   * Clear Dataviews - If selected, the new task will start with no Dataviews 
...
1. In the `Reset Task` modal, if you want the task's artifacts and debug samples to be deleted from the 
   Klique file server, click the checkbox
1. Click **Reset**
...
1. Enqueue the task for execution. Right-click the task > Enqueue > Select a queue > **ENQUEUE**.
   ...
   :::note
   Make sure that a [`clearml-agent`](https://clear.ml/docs/latest/docs/clearml_agent/deployment_bare_metal) has been assigned to the selected queue 
   :::

A Klique Orchestrator will fetch the task from the queue and execute it. The task can now be tracked and its 
results visualized.
