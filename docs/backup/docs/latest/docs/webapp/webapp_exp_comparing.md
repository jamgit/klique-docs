...
> The task comparison page in the WebApp, covering side-by-side details, hyperparameter, scalar, plot, and debug sample comparison modes.

The Web UI provides features for comparing tasks, allowing to locate, visualize, and analyze the 
...
objects, dataviews, and other details.
* Hyperparameters
...
tasks update. Klique also support embedding resources in third-party platforms that support embedded content (e.g. Notion).
...
models, artifacts, dataviews, configuration objects, and additional general information. **Hyperparameters** (Values view) displays the 
...
* Hover and click  on the task that will be the new base.
* Hover and click  on the new base task and drag it all the way to the left

The differences between the tasks are highlighted. Easily locate 
value differences by clicking click 
...
You can download the scalar comparison table as a CSV file by clicking . 
...
between iterations, click  (older images),
 (newer images), 
or  (newest images).
...
To move to the same sample in another iteration, click 
