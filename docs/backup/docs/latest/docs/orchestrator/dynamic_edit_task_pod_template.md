# Dynamic Task Pod Templates

> Using KLIQUE_ORCHESTRATOR_TEMPLATE_MODULE to inject custom Python code that dynamically modifies Kubernetes task pod templates before they run.

On Kubernetes, Klique's Orchestrator allows you to inject
custom Python code to dynamically modify the Kubernetes Pod template before applying it.

## Agent Configuration

The `KLIQUE_ORCHESTRATOR_TEMPLATE_MODULE` environment variable defines the Python module and function inside that 
module to be invoked by the orchestrator before applying a task pod template. 

The orchestrator runs this code in its own context, passes arguments (including the actual template) to the function,
and uses the returned template to create the final Task Pod in Kubernetes.
...
* `template` (Python dictionary) - Base Pod template created from the orchestrator's configuration and any queue-specific overrides.
...
* `worker` - The orchestrator's Python worker object, in case custom calls might be required.

### Usage

Update `orchestrator-values.override.yaml` to include:

```yaml
orchestratork8sglue:
  extraEnvs:
   - name: KLIQUE_ORCHESTRATOR_TEMPLATE_MODULE
...
* When defining a custom code module, by default the orchestrator will start watching pods in all namespaces 
  across the cluster. If you do not intend to give a `ClusterRole` permission, make sure to set the 
  `KLIQUE_ORCHESTRATOR_MONITOR_ALL_NAMESPACES` env to `"0"` to prevent the orchestrator from trying to list pods in all namespaces. 
  Instead, set it to `"1"` if namespace-related changes are needed in the code.

  ```yaml
  orchestratork8sglue:
    extraEnvs:
      - name: KLIQUE_ORCHESTRATOR_MONITOR_ALL_NAMESPACES
        value: "0"
  ```
...
* `KLIQUE_ORCHESTRATOR_POD_PRE_APPLY_CMD` and `KLIQUE_ORCHESTRATOR_POD_POST_DELETE_CMD` env vars let you define custom bash 
  code hooks to be executed around the main apply command by the orchestrator, such as creating and deleting a PVC object.

* `KLIQUE_ORCHESTRATOR_TEMPLATE_MODULE` env var and a file mount let you define custom Python code in a specific context, 
  useful to dynamically update the main Pod template before the orchestrator applies it.

* This example uses a queue named `pvc-test`, make sure to replace all occurrences of it.

* `KLIQUE_ORCHESTRATOR_POD_PRE_APPLY_CMD` can reference templated vars such as `{queue_name}, {pod_name}, {namespace}` that will 
  be replaced with the actual values by the orchestrator at execution time.
...
orchestratork8sglue:
  ...
  extraEnvs:
    ...
    - name: KLIQUE_ORCHESTRATOR_POD_PRE_APPLY_CMD
    ...
    - name: KLIQUE_ORCHESTRATOR_POD_POST_DELETE_CMD
