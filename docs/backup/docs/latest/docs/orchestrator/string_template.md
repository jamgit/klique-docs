# Custom Pod Manifests with String Templates

> The CLEARML_ string template syntax, variables, and filter operators for customizing Kubernetes pod manifests in the orchestrator Helm chart.

When launching a Kubernetes Pod to execute a  Task, the Klique Orchestrator supports string templates. Templates can reference
Klique system objects and runtime configuration values, using the following format: `${CLEARML_<variable-specifier>:<default-value>}`.
...
- `PROVIDERS_INFO` - Contains user data retrieved from the identity provider (SSO), if configured on the Klique Server.
...
## Using Templates in the `orchestrator` Helm Chart

String templates customization is supported under the `basePodTemplate` and `templateOverrides` properties of the
`orchestrator` chart's values.
