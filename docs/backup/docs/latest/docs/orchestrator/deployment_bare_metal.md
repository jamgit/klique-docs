...
> Installing and configuring ClearML Agent on bare-metal servers or VMs, where one process fills both the Orchestrator and Executor roles.

This page explains how to install and deploy ClearML Agent on bare-metal servers or VMs.
On these fabrics, a single ClearML Agent process fills both the Klique Orchestrator and 
Klique Executor roles, both scheduling jobs onto the machine and then running them.
...
It can be installed on this machine independently of agent initialization and does not require a Klique Server connection. 
...
:::note
If a `clearml.conf` file already exists, follow [this](#adding-clearml-agent-to-a-configuration-file) to add 
...
1. In a terminal session, execute
   ...
   ```
    ...
    The setup wizard prompts for API credentials (see [here](../webapp/settings/webapp_settings_profile.md#api-credentials) about obtaining credentials).
    ```
    Please create new credentials through the settings page in your Klique WebApp.
...
1. At the command prompt `Paste copied configuration here:`, paste the API credentials and press **Enter**. 
...
1. **Enter** to accept the default server URL, which is detected from the credentials or enter a web server URL.

   A secure protocol, https, must be used. **Do not use http.**
    
   ```
   WEB Host configured to: [https://app.your-domain.com] 
   ```
...
1. The wizard responds with your configuration:
   ...
   File Store: https://files.your-domain.com
...
1. Input the default output URI. Model checkpoints (snapshots) and task artifacts will be stored in this output location.
   ...
   ```
   Default Output URI (used to automatically store models and artifacts): (N)one/Klique (S)erver/(C)ustom [None]
   ```
...
1. Optionally, configure **ClearML Agent** (default docker, package manager, etc.). See the [ClearML Configuration Reference](https://clear.ml/docs/latest/docs/configs/clearml_conf)
   and the [ClearML Agent Environment Variables reference](http://clear.ml/docs/latest/docs/clearml_agent/clearml_agent_env_var/). 
   
:::note
The Klique Server provides a [configuration vault](../webapp/settings/webapp_settings_profile.md#configuration-vault), the contents 
...
To completely separate the compute fabric from your workloads, it is recommended to deploy on an orchestrated fabric
such as [Kubernetes](deployment_k8s.md), where the Klique Orchestrator and Klique Executor roles run as
separate components.

## Spinning Up a ClearML Agent
...
:::note[Docker Mode]
Make sure to include the `--docker` flag, as GPU management through the agent is only supported in
...
To split a GPU across multiple tasks instead of allocating whole GPUs, see
[Fractional GPUs](fractional_gpus/bare_metal_dynamic_fractional_gpus.md).
