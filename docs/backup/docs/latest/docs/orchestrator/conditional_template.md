...
> How to configure Conditional Templates to apply Kubernetes pod template overrides only when a specified Task field condition matches.
...
Conditional templates are configured through the `KLIQUE_ORCHESTRATOR_COND_CUST_TEMPLATE` environment variable, whose value must be in
...
orchestratork8sglue:
  extraEnvs:
    - name: KLIQUE_ORCHESTRATOR_COND_CUST_TEMPLATE
