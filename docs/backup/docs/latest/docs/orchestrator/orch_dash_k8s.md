# Orchestration Dashboard Customization (K8s)

> Configuring GPU and CPU capacity reporting, worker ID templates, and hidden worker patterns for the Klique Orchestration Dashboard on Kubernetes.

The Klique [Orchestration Dashboard](../webapp/webapp_orchestration_dash.md) provides visibility into available and in-use compute resources across your 
infrastructure. The Klique Orchestrator's configuration controls how 
resources are reported to the dashboard and how workers are organized into categories and groups.

## Reporting Cluster Capacity

### GPU Capacity

To show the GPUs available to the orchestrator in the Orchestration Dashboard,
configure the orchestrator in one of the following ways:
...
* **Automatic GPU Discovery**:
  ...
  Enable automatic discovery to have the orchestrator detect and report GPUs across your Kubernetes cluster.
  This requires cluster-level access (`orchestratork8sglue.serviceAccountClusterAccess: true`), since the orchestrator must list and evaluate all cluster nodes.

  When GPU discovery is enabled, the orchestrator identifies which cluster nodes have GPUs and how many GPUs each node provides.
  ...
  * `gpuCountSelector` - Comma-separated list of labels used to count GPUs per node. The first matching label determines the reported GPU count.
  ...
  With discovery enabled, the orchestrator evaluates nodes matching the provided selectors and reports their GPU 
...
To show the CPUs available to the orchestrator in the Orchestration Dashboard,
configure the orchestrator in one of the following ways:
...
* **Automatic CPU Discovery**:
  ...
  Enable automatic discovery to have the orchestrator detect and report CPUs across your Kubernetes cluster.
  This requires cluster-level access (`orchestratork8sglue.serviceAccountClusterAccess: true`), since the orchestrator must list and evaluate all cluster nodes.
...
* **Namespace-scoped CPU Reporting**:
  ...
  To report CPUs scoped to a single namespace, enable `reportNamespaced`. By default, the orchestrator's own namespace is used. Use `reportNamespaceOverride` to specify a different namespace.
...
* `reportType` - How the orchestrator sends reports to the dashboard. Use one of the following options:
  * `disabled` (or no value) - Do not send any reports. 
  * `global` - Send a single category-level report that sums up all orchestrators into the category total. Overrides individual 
  orchestrator reports. For more information about agent categorization, see [Resource Categories and Groups](../webapp/webapp_orchestration_dash.md#resource-categories-and-groups). 
  * `aggregate` - Send a report per orchestrator. The dashboard aggregates all reports in the category automatically. For more information about agent categorization, see [Resource Categories and Groups](../webapp/webapp_orchestration_dash.md#resource-categories-and-groups).
* `reportSeconds` - Interval in seconds between dashboard updates. Controls how frequently the orchestrator sends capacity data.
...
The following defaults are used by the `orchestrator` helm chart: 
* Category: `k8s`
* Resource Group: `group` 
* Workload ID: Makes use of the Task ID of the task executed in that node. How the agents appear in the dashboard can be 
  customized by chart overrides (`orchestrator-values.override.yaml`):
...
`control-plane-values.override.yaml` file:
