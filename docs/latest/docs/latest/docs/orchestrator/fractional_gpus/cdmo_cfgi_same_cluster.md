# Install CDMO and CFGI on the Same Cluster

> Configuring the NVIDIA GPU Operator with multiple device plugin configurations to run CDMO and CFGI together on one Kubernetes cluster.

You can install both CDMO (Cluster Dynamic MIG Operator) and CFGI (Cluster Fractional GPU Injector) on a shared Kubernetes cluster. 
