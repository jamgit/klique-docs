...
> Deploying the NVIDIA GPU Operator on Kubernetes with the recommended configuration for use with Klique's fractional GPU features.

This guide provides recommended configuration values for deploying the NVIDIA GPU Operator alongside Klique. 
...
* [Cluster Dynamic MIG Operator](cdmo.md) (CDMO) – Dynamically configures MIG GPUs on supported devices.
* [Cluster Fractional GPU Injector](cfgi.md) (CFGI) – Enables fractional (non-MIG) GPU 
