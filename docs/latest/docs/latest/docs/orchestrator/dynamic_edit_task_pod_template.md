...
> Using CLEARML_K8S_GLUE_TEMPLATE_MODULE to inject custom Python code that dynamically modifies Kubernetes task pod templates before they run.
...
* When defining a custom code module, by default the orchestrator will start watching pods in all namespaces 
  ...
  `CLEARML_K8S_GLUE_MONITOR_ALL_NAMESPACES` env to `"0"` to prevent the orchestrator from trying to list pods in all namespaces. 
