...
| `assignable` | Whether admins can assign users to this group from the Klique Web UI (`true`/`false`). If `false`, group membership is managed externally or implicitly. | `CLEARML__services__auth__default_groups__<GroupName>__assignable` | `"false"`                       |
...
credentials for `controlPlane.apiKey` and `controlPlane.apiSecret` (or `controlPlane.existingSecret`).
:::
