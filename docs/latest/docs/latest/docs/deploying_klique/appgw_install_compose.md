...
> Docker-compose installation of the AI Application Gateway and the LLM Gateway it fronts, for self-managed Klique Server and workload node setups.

The AI Application Gateway enables external HTTP(S) or direct TCP access to Klique tasks and applications running on
...
This guide describes how to install and run the Klique AI Application Gateway using docker-compose for environments
where you manage both the Klique Server and the workload nodes.
...
* Root access
* Credentials for the Klique container repository
...
Use the Klique DockerHub credentials when prompted by `docker login`.

### Docker-compose File

This is an example of the `docker-compose` file you will need. Alongside the `ai-gateway-proxy` and `ai-gateway-router`
services, it runs the two [LLM Gateway](#llm-gateway) containers, `ai-gateway-llm` and `ai-gateway-secrets-init`:
...
* `PROXY_TAG`: AI Application Gateway proxy tag. The Docker image tag for the proxy component, which needs to be
  specified during installation. This tag is provided by Klique to ensure compatibility with the recommended version.
* `ROUTER_TAG`: App Gateway Router tag. The Docker image tag for the router component. It defines the specific version
  to be installed and is provided by Klique as part of the setup process.
* `LLM_GATEWAY_TAG`: [LLM Gateway](#llm-gateway) data plane tag. The image tag used by the `ai-gateway-llm`
  and `ai-gateway-secrets-init` containers, provided by Klique.
...
* `CLEARML_API_HOST`: Klique API server URL starting with `https://api.`
* `CLEARML_API_ACCESS_KEY`: Klique server API key.
* `CLEARML_API_SECRET_KEY`: Klique server secret key.
* `AUTH_COOKIE_NAME`: Cookie used by the Klique server to store the Klique authentication cookie. This can usually be
  found in the `envoy.yaml` file in the Klique server installation (`/opt/allegro/config/envoy/envoy.yaml`), under the
...
Klique WebApp, under **Settings > Application Gateway**.
