...
* To use a custom defined `imagePullSecret` for the **Klique Server**, configure the following in your `control-plane-values.override.yaml` file. The `global.imagePullSecrets` and `mongodb-kubernetes.registry.imagePullSecrets` overrides are also required, so the dependency charts stop referencing the generated `klique-registry-key` secret:
...
* `gateway` (AI Application Gateway):
   ...
   helm template oci://docker.io/kliqueai/gateway \
     --set controlPlane.apiKey=dummy \
     --set controlPlane.apiSecret=dummy \
     --set controlPlane.apiServerUrlReference=dummy | yq '..|.image? | select(.)' | sort -u
   ```
