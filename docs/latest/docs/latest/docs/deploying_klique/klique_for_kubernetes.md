...
- [Platform Management Center](extra_configs/platform_management_center_deploy.md) installs the `platform-management-center`
...
- [Event Metering](extra_configs/event_metering.mdx) turns on the usage metering service through the
  `usageAggregator` setting in the `control-plane` chart, and deploys the standalone `compute-usage-metering`
