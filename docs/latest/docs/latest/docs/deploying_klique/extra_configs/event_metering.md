...
The `control-plane` chart provides built-in [control plane event reporters](#control-plane-usage-events). An additional built-in reporter, [`compute-usage-metering`](#compute-resources-events), is provided as a standalone Helm chart and reports Kubernetes compute usage from a workload cluster.
...
To report compute usage from a Kubernetes workload cluster to the metering service, deploy the standalone `compute-usage-metering` Helm chart on the workload cluster.
