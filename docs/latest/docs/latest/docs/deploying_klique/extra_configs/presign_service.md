...
Create a `presign-service-values.override.yaml` file (make sure to replace the placeholders):
...
```bash
helm upgrade -i -n klique presign-service oci://docker.io/kliqueai/presign-service -f presign-service-values.override.yaml
```
