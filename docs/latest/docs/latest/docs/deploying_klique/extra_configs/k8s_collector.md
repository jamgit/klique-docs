...
> The resource-metrics-collector Helm chart, which scrapes Kubernetes cluster metrics and forwards them to the Klique Server's Hardware Dashboard.

The `resource-metrics-collector` Helm chart scrapes cluster metrics and forwards them to the Klique Server for display in the 
...
```shell
helm upgrade -i -n resource-metrics-collector --create-namespace resource-metrics-collector \
  oci://docker.io/kliqueai/resource-metrics-collector \
...
| `excludeNamespacesByRegex` | ```^(kube-system\|…\|klique\|clearml)$``` | Drops pod/container metrics by namespace. System namespaces and the default `klique` namespace are dropped by default. If you installed the control plane into a different namespace, check that it is matched: if it is not, Klique's own pods are reported as workload. |
