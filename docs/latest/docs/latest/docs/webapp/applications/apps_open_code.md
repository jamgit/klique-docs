...
> A detachable remote VS Code session pre-configured with the OpenCode CLI, with token usage metered through the LLM Gateway.

The OpenCode app launches a detachable remote VS Code session pre-configured with OpenCode's CLI, ready to use as
soon as the session starts.

The OpenCode app offers workspace management features, allowing you to store, sync, and restore interactive workspaces
across sessions. This ensures your work, including `~/.config/opencode` session state, is preserved and can be easily
accessed in future sessions.

The OpenCode session is set up using a [Klique Orchestrator](../../orchestrator.md). When configuring an app instance, select a queue, and the 
Klique Orchestrator servicing that queue will set up the remote environment. When setup is complete, the app instance dashboard displays 
the session ID and a link to open the VS Code session with OpenCode ready to use.
...
Once you start an OpenCode instance, you can view the following information in its dashboard:
...
* Current session ID
* Open OpenCode - Link to the VS Code session with OpenCode ready to use
...
already available. Ask OpenCode to work with your Klique projects, tasks, models, and apps directly, with no manual
connection or installation step.
:::
...
When configuring a new OpenCode instance, you can fill in the required parameters or reuse the configuration of 
...
* **Compute Resource (Queue)** – The [Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) serviced by the Klique Orchestrator that will run the OpenCode session.
* **OpenCode Session Project** - The project where the app instance is created. Access follows project-level 
...
* **AI Provider**
  * Vendor Endpoint - Route model calls through the [LLM Gateway](../settings/webapp_settings_llm_gateway.md) instead 
    of calling the AI provider's API directly. Leave blank to call the API directly. 
  * Set a Custom OpenCode API Key - Provide a custom OpenCode API key for the session instead of the default key from 
    your [User Vault](../settings/webapp_settings_profile.md#configuration-vault).
  * Update OpenCode at Session Start - When enabled, updates the OpenCode CLI to the latest version when the 
...
* **Advanced Options**
  ...
  * Continue from Previous Session - Select the ID of a previous OpenCode session to restore
  * Container Language - Base runtime image for the session container. `Full` includes Python, Node.js, Rust, and Go 
    at their latest versions; `Rust` and `Go` provide a single-language image; select `Custom` to provide your own 
    image from a registry instead.
  * Environment Variables - One `KEY=value` per line. Injected into every shell inside the session via 
    `/etc/profile.d/opencode.sh`. Lines starting with `#` are ignored.
  ...
  * Persistent Workspace Path - Directory snapshotted when the session closes and loaded back in when a new 
    ...
    `~/.config/opencode` state, and cloned repos all persist across sessions. Leave blank for an ephemeral session.
