...
> The Streamlit Launcher application for serving Streamlit apps on remote machines, its dashboard, and instance configuration options.

[Streamlit](https://streamlit.io/) is a framework for creating visual web interfaces for your models. The Klique Streamlit 
...
The Streamlit Launcher relies on the Klique App Gateway Router which implements user authentication, and redirects requests 
to the IP/port served by the Streamlit app. 

If the Klique AI Application Gateway is not available, the Streamlit app might not be accessible.
...
* Streamlit App status indicator
  *  - App is running and is actively in use
  *  - App is setting up 
  *  - App is idle
  *  - App is stopped 
* Idle time 
...
* **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the Streamlit launcher 
...
* **Docker Image** - Docker image the Klique Executor will use for running the Streamlit app
