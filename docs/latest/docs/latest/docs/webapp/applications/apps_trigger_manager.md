...
> The Trigger Manager application for running tasks automatically when dataset, model, or task events occur, its dashboard, and configuration.

The Trigger Manager Application lets you define tasks to be run when predefined events occur. The Trigger Manager 
...
Each trigger is configured to run a task when its specifications are met. 
...
* **Triggers** - Events whose occurrence will cause a task to be executed
  ...
  * Queue - The Klique Queue to which cloned tasks are enqueued (make sure an agent is assigned to that queue)
