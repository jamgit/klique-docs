...
> The AWS Autoscaler application, which spins EC2 instances up and down based on queue demand and a configured resource budget.
...
Each resource type is associated with a Klique [queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) whose status determines the need for instances of that resource 
...
For more information about how autoscalers work, see [Autoscalers Overview](../../orchestration_engine/cloud_autoscaling.md#available-options).
...
* **Git Configuration** - Git credentials with which the Klique Executors running on your EC2 instances will access your 
...
* **Use docker mode** - If selected, tasks enqueued to the autoscaler will be executed by Klique Executors running in 
[Docker mode](https://clear.ml/docs/latest/docs/clearml_agent/clearml_agent_execution_env/#docker-mode) 
  * **Base Docker Image** (optional) - Available when `Use docker mode` is selected: Default Docker image in which the 
  Klique Executors will run. Provide an image stored in a Docker artifactory so instances can automatically fetch it
* **Compute Resources**
...
* **Apply Task Owner Vault Configuration** - Select to apply values from the task owner's [configuration vault](../settings/webapp_settings_profile.md#configuration-vault) when executing the task 
...
* **Ignore vault parsing errors** - If not selected, the autoscaler will abort if encountering errors when loading vaults 
  ...
  tasks run by the autoscaler. For more information, see [Configuration Vault](#configuration-vault).
* **Init script** (optional) - A bash script to execute after launching the EC2 instance 
* **Additional ClearML Configuration** (optional) - A `clearml` configuration file to use by the Klique Executors when 
...
and [AWS API Reference: RunInstances](https://docs.aws.amazon.com/AWSEC2/latest/APIReference/API_RunInstances.html)
* **Run with Service Account** -  Select to allow running the application under a [Service Account](../settings/webapp_settings_users.md#service-accounts) identity instead of under your own identity
...
* `extra_clearml_conf` - `clearml` configuration to use by the Klique Executor when executing your tasks. This 
...
* Run the autoscaler using a [Klique Service Account](../settings/webapp_settings_users.md#service-accounts). Add the 
...
* Admins can add the configuration to a [Klique Administrator Vault](../settings/webapp_settings_admin_vaults.md)
...
To support workloads that require nested container environments, such as the Klique [Virtual Machine Remote Desktop](apps_vm_desktop.md) 
