...
> A standalone, detachable Milvus vector database session with automatic serialization and workspace restore.
...
If the Klique AI Application Gateway is not available, the database endpoint might not be accessible. For more information, 
...
* **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the Milvus DB session 
...
* **Advanced Options**
  * Database Session Project - The Klique project where the app instance is created. Access is determined by project-level permissions (i.e. users with read access can use the database session)
