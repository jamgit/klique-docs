...
> A standalone, detachable Qdrant vector database session with automatic serialization and workspace restore.
...
* **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the Qdrant DB session 
...
* **Advanced Options**
  * Database Session Project - The Klique project where the app instance is created. Access is determined by project-level 
