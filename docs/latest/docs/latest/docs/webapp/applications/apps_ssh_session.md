...
> The SSH Session application for launching a remote development environment with an SSH session, its dashboard, and configuration options.
...
The SSH Session is set up using a [Klique Orchestrator](../../orchestrator.md). When configuring an app instance, select a 
queue, and the Klique Orchestrator servicing that queue will set up the remote environment and SSH session. When the environment setup is 
...
* **Queue** - The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the SSH Session app 
...
* **Advanced Options**
  * Interactive Session Project - The Klique project in which the interactive session is created. If left empty, the 
