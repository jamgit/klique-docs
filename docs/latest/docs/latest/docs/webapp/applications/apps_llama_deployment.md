...
> GGUF format LLM deployment with llama.cpp behind a secure endpoint, covering multi-model hosting, CPU/disk offloading, and engine configuration.
...
* **Queue**: The [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) to which the 
...
* **Model Configuration**: Configure the behavior and performance of the model serving engine.   
  ...
  * Model: A Klique Model ID or a Hugging Face model. The model must be in GGUF format. If you are using a HuggingFace model, 
