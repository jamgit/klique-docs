...
> The VS Code application for launching a remote VS Code session, its dashboard, and instance configuration options.
...
The VS Code session is set up using a [Klique Orchestrator](../../orchestrator.md). When configuring an app instance, 
...
* **Queue** - The queue serviced by the Klique Orchestrator that will execute the VS Code session
