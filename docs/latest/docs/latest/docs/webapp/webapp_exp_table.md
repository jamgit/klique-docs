...
> The WebApp task table, including creating, filtering, customizing, comparing, and performing actions on tasks.

The task table is a [customizable](#customizing-the-task-table) list of tasks associated with a project. From the tasks
table, view task details, and work with tasks (reset, clone, enqueue, create [tracking leaderboards](#creating-a-task-leaderboard)
...
Use the search bar  
to find specific tasks. You can query by: 
* Task name
* Task ID
* Description
...
run it through a [Klique](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) or save it as a *draft*.
...
1. In the `Create Task` modal, input the following information:
   ...
   * **Arguments** (*optional*) - Add [hyperparameter](https://clear.ml/docs/latest/docs/fundamentals/hyperparameters) values.
   ...
   * **Run**
     * Queue - [Klique Queue](https://clear.ml/docs/latest/docs/fundamentals/agents_and_queues/#what-is-a-queue) where the task should be 
     enqueued for execution
     * Output Destination - A URI where task outputs should be stored (Klique file server by default). 
...
|---|---|---|
| **TYPE** | Type of task. Klique supports multiple [task types](https://clear.ml/docs/latest/docs/fundamentals/task#task-types) for experimentation, and a variety of workflows and use cases. | Default |
...
bookmark and shared with other Klique users to collaborate.
...
| Archive | Move task to the project's archive. | Any state | *Pending* to *Draft*                            |
...
:::important[Custom UI Context Menu Actions]
The Klique Server provides a mechanism to define your own custom actions, which will 
