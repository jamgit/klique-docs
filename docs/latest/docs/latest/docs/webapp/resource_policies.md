...
> Resource policies for defining user group resource quotas and reservations across Klique queues.
...
* Connect resource profiles to a policy, making them available to its user group via Klique queues
...
1. In the **Connect Profile** modal, input the following information:
    * Queue name - The name for the Klique queue the policy’s users will use to enqueue jobs using this resource 
...
* Resource consumption breakdown - Click 
...
1. On the resource policy card, click  **> Edit**
...
1. On the resource policy box, click  
2. Click **Delete**
