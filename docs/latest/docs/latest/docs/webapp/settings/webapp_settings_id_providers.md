...
> Administrators connect OIDC and SAML identity providers to enable SSO login to the Klique WebApp.

Administrators can connect identity service providers to the server: configure an identity connection, which allows
Klique to communicate with your identity provider, retrieve user information, authenticate users, and more. Once an identity 
...
4. Provide the **Provider Connection Configuration**. Note that some of the information, such as the `Client ID`,
   `Client Secret`, and the `Authorization Endpoint`, must be obtained from your identity provider's Klique App settings
   and copied to the relevant field.
   
   
   <summary class="panel-title">OIDC Providers</summary>

      * Klique WebApp URL - The external URL for the Klique WebApp. Used to construct the Callback URL used by the 
      identity provider
      * Callback URL - Copy the field's content into the `Callback URL` field in your provider's Klique App / integration
      (sometimes the field is called `Redirect URL`). Used by the identity provider to redirect the session back to the Klique 
      Server. 
      * Display name - Unique name to be used in the Klique Login screen. Make sure no two 
      connections have the same name.
      * Client ID - The Client ID of your identity provider's Klique App. Copy this value from your identity provider's 
       Klique App settings
      * Client Secret - The client secret generated by your identity provider for the Klique App. Copy this value from your identity 
       provider's Klique App settings
      * Authorization Endpoint - Used to authorize each user login. Copy this value from your identity provider's Klique App settings
      * Access Token Endpoint - Used to obtain an access token as part of the login process. Copy this value from your 
       identity provider's Klique App settings
      ...
      * Obtain user information from Identity Token - Select to retrieve user information from the identity token generated 
       ...
       as part of the login process. Copy this value from your identity provider's Klique App settings 
      ...
      * Create new users on first login - Select to automatically create a Klique user for new users logging in using 
       this connection. If not selected, 
       only existing Klique users or users who were invited using their email address can log in using this connection.
      * Verify token signature - Select to verify 
       ...
       identity provider's Klique App is set up to sign tokens. 
      * Additional Settings
        * Code challenge method - Select one of the methods from the dropdown menu (`None`, `Plain`, or `S256`). This setting 
         should match the one defined in your identity provider's Klique App settings
      ...
      * Claims mappings - Maps Klique claim names to the names defined in your identity provider's Klique App settings
      ...
      * User Groups Synchronization - Supports synchronizing group membership. If enabled, and the groups claim is returned 
       by the provider, the server will attempt to match each group to an existing Klique group and assign the Klique user 
       to the matched Klique group. The server will also make sure the Klique user is removed from the group if 
   ...
   <summary class="panel-title">SAML Providers</summary>
      ...
      * Klique Webapp URL - The external URL for the Klique WebApp. This is used to construct the Callback URL used by 
       the identity provider
      * Reply/ACS URL - Copy this field's content into the `Assertion Consumer Service URL` field in your provider's 
       Klique App. Used by the identity provider to redirect the session back to the Klique Server. 
      * Display name - Unique name to be used in the Klique Login screen. Make sure no two 
       connections have the same name. 
      * Entity ID - The Entity ID of your identity provider's Klique App. Copy this value from your identity provider's 
       Klique App settings 
      * IDP metadata file - Provided by the identity provider and contains SAML configuration details used by the Klique 
       Server to set up the connection. Download this from your identity provider's Klique App settings. 
      * Security Settings
        * Require signed assertions - Assume IDP assertions or responses are signed by the identity provider. If enabled, 
         make sure your identity provider's Klique App is configured to sign assertions or responses.
        * Sign client requests - The Klique Server will sign requests sent to the identity provider. Make sure your 
          identity provider's Klique App is configured to accept signed client requests and its Client Certificate section 
          ...
          file into the `Private Key` field below. Make sure your identity provider's Klique App is configured to accept 
