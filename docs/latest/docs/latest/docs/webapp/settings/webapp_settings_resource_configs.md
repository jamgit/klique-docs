...
> Resource Configuration settings define resource pools and profiles that allocate compute across user groups via resource policies.
...
To utilize a resource pool for task execution, `clearml-agent` instances are assigned to the pool servicing its specific queues. 
Tasks assigned by the resource policy manager to that pool will be sent to these queues for execution.

To configure the Klique Orchestrator for a resource pool:

1. Open the resource pool’s information panel by clicking  on the pool card. This displays:   
...
2. Copy the ID for the queue you want the Klique Orchestrator to service.  
3. Configure the Klique Orchestrator to listen to the queue. For example, on Kubernetes:   
   ```yaml
   orchestratork8sglue:  
     queues:  
   ...
   You must explicitly configure the Klique Orchestrator (for example, by setting GPU limits in `templateOverrides`) to match 
...
`pool A` cannot currently satisfy its resource requirements).
