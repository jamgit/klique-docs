...
> Storage Cleanup settings configure Google Cloud, AWS S3, and Azure credentials used to delete task artifacts when tasks are deleted.

To enable deletion of task artifacts stored in cloud storage when a task is deleted, administrators can configure 
access credentials for storage providers in use:
