...
> Application Gateway settings monitor routers, define static routes, and manage access tokens for external endpoint access to compute node jobs.

The [AI Application Gateway](../../whats_klique/appgw.md) facilitates setting up secure, 
authenticated access to jobs running on your compute nodes from external networks (see application gateway installation
for [Kubernetes](../../deploying_klique/appgw_install_k8s.md), Docker-Compose for [Self-Hosted Deployment](../../deploying_klique/appgw_install_compose.md) 
or Docker-Compose for [Hosted Deployment](../../deploying_klique/appgw_install_compose_hosted.md)).
...
* **Info**: General router information
  * Router details 
    ...
    * Last update time 
    * Router version 
  * Routed Tasks table: Tasks currently available for access through the router
...
* **Test Details**: Administrators can run a test to verify that a gateway is functioning correctly: Identifying routed 
  ...
  (specified through the desired Klique queue), and checks that the router successfully creates a route to that task, 
  ...
  :::note
  Testing is only supported when both the WebApp and the gateway endpoint are served over secure (HTTPS) protocols.
  :::
  ...
  * Worker - Klique Executor that executed the test task
  * Test Task name
  * Task ID 
  * Browser endpoint
  * Endpoint 
...
## Static Routes
**Static Routes** allow administrators to define external endpoints which users can attach to their Klique application 
...
1. In the **Add New Route** modal, input the following information 
   ...
   * Authenticated Route - Whether the route is accessible to specific users (default) or is publicly accessible.
     * Endpoint Access - When authentication is required, add the Klique user groups whose members can access this endpoint.

### Editing Routes
A static route can be modified only when it is disabled. To edit route details, hover and click **Edit**  
in the **Static Routes** table.

An enabled route is view-only. To view its details, hover and click **View route configuration**  
in the **Static Routes** table.

### Deleting Routes
To delete a static route details, hover and click **Delete**  
...
* User tokens - Grant access according to Klique user (or service account) identity privileges
...
2. Select the token type and enter token details:
   * User token
     ...
     * `User` - The Klique user identity whose privileges will be used for authorizing endpoint access
