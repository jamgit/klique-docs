...
> The pipeline run details panel, showing pipeline structure, stages, step information, and run and step details tabs.
...
details panel, click **DETAILS** and then click on a step node, or hover over a step node and click .
...
Click  on the details panel header to view the panel in full screen.
