...
> The Projects Page for navigating project folders, filtering, configuring project settings, and creating new projects.
...
* When a project's `default_output_destination` is set, the folder displays the  
...
By default, Klique infrastructure projects (i.e. dataset, pipeline, reports, application projects) are not shown in the 
...
* **Project Settings** 
  ...
  * **Storage Mounts** - Specify storage configuration the Klique Orchestrator will apply when running tasks 
