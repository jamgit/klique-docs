...
> The WebApp model table, including its columns, customization, filtering, tagging, and model actions.
...
View the model table in table view  
or in details view ,
...
You can download the model table as a CSV file by clicking  
...
can be saved in a browser bookmark and shared with other Klique users to collaborate.
...
* In the model table, right-click a model, or hover over a model and click  to 
...
| Action | Description | States Valid for the Action |
|---|---|--|
...
* To Add tags and to change tag colors:
    1. Click the task **>** Hover over the tag area **>** **+ADD TAG** or 
       (menu)
