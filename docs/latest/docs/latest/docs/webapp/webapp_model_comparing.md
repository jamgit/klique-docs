...
> The model comparison page in the WebApp, covering side-by-side details, tabular scalar, and plot comparison modes.

The Web UI provides features for comparing models, allowing to locate, visualize, and analyze model differences. 
...
models update. Klique also supports embedding resources in third-party platforms that support embedded content (e.g. Notion).
...
* Hover and click  on the new base model and drag it all the way to the left
