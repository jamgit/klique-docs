...
> The dataset versioning WebApp for viewing, creating, filtering, and managing hyper-dataset versions and their frames.
...
To view the details of a specific frame, click on its preview, which will open the [Frame Viewer](datasets_frames.md#frame-viewer).
...
**To apply filters:**
1. In the **FRAMES** tab, click  (**Filters**).
1. In a **FRAME FILTER**, create one of the following rules:
   * ROI rule - Use "Include" and "Exclude" conditions to match frames according to an ROI label. If the "Include" 
   ...
   if it contains at least one annotation matching each rule, even if the annotations are in different ROIs. Click  
